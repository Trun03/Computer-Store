...
class clsSanpham2
{
	...
	function LayDanhSachSanpham($trangthai=2, $cat_id=0, $tukhoa="")
	{
		$sql = "SELECT Sp.*, Cat.cat_name, Cat.cat_status 
					FROM part AS Sp INNER JOIN tbCategory2 AS Cat 
	...
	{
		$sql = "UPDATE part SET title=?, price = ?, 
	...
	{
		$sql = "SELECT Sp.*, Cat.cat_status FROM part AS Sp INNER JOIN tbCategory2 AS Cat 
	...
	{
		$sql = "UPDATE part SET status = ? WHERE id=?";
		$data[] = $status;
		$data[] = $id;
 		$ketqua = $this->db->ThucthiSQL($sql,$data);
		return $ketqua;
	...
	{
		$sql = "SELECT * FROM part WHERE id in ($list)";
		if($trangthai!=2) 
			$sql = $sql . " AND status = " . $trangthai;
